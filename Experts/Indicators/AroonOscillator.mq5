...
#include <Okmich\Indicators\AroonOscillator.mqh>

//--- input parameters
input int InpPeriod=14;
input ENUM_ARNOSC_Filter_Type InpSignalType = ARNOSC_CROSS_MID_LEVEL;
input int InpVolumeMultiples=1; //Lot size mulitples
...
private :
   ...
   int   mPeriod;
   //--- indicators
   CAroonOscillator  *m_AroonOscillator;
   //--- indicator buffer
   //-- others


public:
                     CAroonStrategry(string symbol, ENUM_TIMEFRAMES period, int aroonPeriod, int InptVolumeMultiples): CStrategy(symbol, period)
     {
      mPeriod = aroonPeriod;
      mLotSize = InptVolumeMultiples*SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
     };
...
//--- m_AroonOscillator
   m_AroonOscillator = new CAroonOscillator(mSymbol, mTimeframe, mPeriod);
   return m_AroonOscillator.Init();
  }
...
void CAroonStrategry::Release(void)
  {
   m_AroonOscillator.Release();
   delete m_AroonOscillator;
  }
...
void CAroonStrategry::Refresh(void)
  {
   if(IsNewBar())
     {
      bool refreshed = m_AroonOscillator.Refresh();
      //--- take values from indicator
      mEntrySignal = m_AroonOscillator.Filter(InpSignalType);
     }
  }

// the expert to run our strategy
CSingleExpert singleExpert(ExpertMagic, "AroonOscillator");
//+------------------------------------------------------------------+
...
int OnInit()
  {
   CAroonStrategry *strategy =
      new CAroonStrategry(_Symbol, _Period, InpPeriod, InpVolumeMultiples);
