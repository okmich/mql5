...
#include <Okmich\Indicators\AwesomeOscillator.mqh>

//--- input parameters
input group "********* Strategy settings *********";
input ENUM_AO_Strategies InpHowTo = AO_Saucer; //Entry option

input group "********* Position management settings *********";
input ENUM_POSITION_MANAGEMENT InpPostManagmentType = POSITION_MGT_FIXED_SL_TRAIL_TP_POINTS;  // Type of Position Management Algorithm
input int InpATRPeriod = 14;                         // ATR Period
input double InpStopLossPoints = 1500;               // Stop loss distance
input double InpBreakEvenPoints = 200;               // Points to Break-even
input double InpMaxFloatingPoints = 1500;            // Maximum floating points/Take profit points
...
//--- m_CiAO
   m_CiAO = new CAwesomeOscillator(mSymbol, mTimeframe, 6);
   return m_CiAO.Init();
  }
   ...
   if(IsNewBar())
     {
      ...
      m_CiAO.Refresh(mRefShift);
      mEntrySignal = m_CiAO.TradeSignal(InpHowTo);
     }
  }
