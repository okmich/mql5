...
private :
   ...
   int               mFFastEmaPeriod, mFastEmaPeriod, mMidEmaPeriod, mSlowEmaPeriod, mCciPeriod;
...
public:
             ...
             int InptCciPeriod): CStrategy(symbol, period)
     {
      mLotSize = InptVolMultiple*SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
      
      mMaMethod = InptMaMethod;
      mFFastEmaPeriod = InptMa1;
      mFastEmaPeriod = InptMa2;
      mMidEmaPeriod = InptMa3;
      mSlowEmaPeriod = InptMa4;
...
bool CStrategyImpl::Init(ulong magic)
  {
//--- m_DblMa
   m_FastDblMa = new CDoubleMovingAverages(mSymbol, mTimeframe, mFFastEmaPeriod, mFastEmaPeriod, mMaMethod);
   m_SlowDblMa = new CDoubleMovingAverages(mSymbol, mTimeframe, mMidEmaPeriod, mSlowEmaPeriod, mMaMethod);
//--- mCci
   bool cciCreated = mCci.Create(mSymbol, mTimeframe, mCciPeriod, PRICE_TYPICAL);

//--- price buffers
   ArraySetAsSeries(m_CloseBuffer, true);

   return m_FastDblMa.Init() && m_SlowDblMa.Init() && cciCreated;
  }
...
int OnInit()
  {
   CStrategy *strategy = new CStrategyImpl(_Symbol, InpTimeframe, InpLotSizeMultiple, InpFFastEmaParam, InpFastEmaParam,
                                InpMidEmaParam, InpSlowEmaParam, InpEmaMethod, InpCciParam);
