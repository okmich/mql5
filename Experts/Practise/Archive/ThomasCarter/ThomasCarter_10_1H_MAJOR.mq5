...
//--- input parameters
const ulong EXPERT_MAGIC = 98760010;
...
input int      InpFastEmaParam=3;
input int      InpSlowEmaParam=15;
...
input int      InpDpoPeriodParam=12;
...
private :
   ...
   int               mMacdShortEmaPeriod, mMacdLongEmaPeriod, mMacdSignalPeriod;
...
public:
             ...
             int InptDmiPeriod=14, int InptAdxPeriod=14, double InptAdxSlopeLevel=2.0, double InptTrendThresh=25.0,
             int InptShortMa=3, int InptLongMa=15, ENUM_MA_TYPE InptMaMethod=MA_SMA,
             int InptMacdShort=12, int InptMacdLong=26, int InptMacdSignal=9,
             int InptDpoPeriod=12): CStrategy(symbol, period)
     {
      ...
      mMacdShortEmaPeriod = InptMacdShort;
      mMacdLongEmaPeriod = InptMacdLong;
      mMacdSignalPeriod = InptMacdSignal;

      mDpoPeriod = InptDpoPeriod;
     };
...
//--- m_Macd
   m_Macd = new CMacd(mSymbol, mTimeframe, mMacdShortEmaPeriod, mMacdLongEmaPeriod, mMacdSignalPeriod);
//--- m_Dpo
   m_Dpo = new CBoundedDpo(mSymbol, mTimeframe, mDpoPeriod);

//--- price buffers
   ArraySetAsSeries(m_CloseBuffer, true);

   return m_Adx.Init() && m_DblMa.Init() && m_Dpo.Init() && m_Macd.Init();
  }
...
int OnInit()
  {
   CStrategy *strategy = new CStrategyImpl(_Symbol, InpTimeframe,InpLotSizeMultiple,
                                InpDmiPeriod, InpAdxPeriod, InpSignfSlope, InpTrendLevel,
