//+------------------------------------------------------------------+
//|                                                  SwingTrader.mq5 |
//|                                Copyright 2018, Algotrading.co.za |
//|                                     http://www.algotrading.co.za |
//+------------------------------------------------------------------+

//+------------------------------------------------------------------+
#property copyright "Algotrading."
#property link      "http://www.algotrading.co.za"
#property version   "1.00"
#property description   "Swing Trader"
//---
...
input string   inpComment           = "Swing Trader V1"; //Trade Comment
input ulong    m_magic=12520191007;                 // magic number
input bool     InpPrintLog          = false;     // Print log
input ENUM_TIMEFRAMES      InpChannelTF =PERIOD_D1;// Channel TF
input int      InpNoOfChannelBars      = 10;                         // No of bars in channel
input int      InpChannelDistance      = 50;     // Distance above/below channel to place order in pips
input uchar    InpMaxBuyPositions   = 1;         // Max Buy Positions
input uchar    InpMaxSellPositions  = 1;         // Max Sell Positions
input int      InpCandleShift    = 5;     // Candle shift to determine highs and lows for the Channel TF
input int      InpRetracementLength     = 10; // Pullback in pips to signal a retrcement or swing
input bool     InpAddPendingOrders     = true;     // Add extra pending orders
input bool     InpReverseBuys     = false;     //Reverse buy trades
input bool     InpReverseSells     = false;     //Reverse sell trades



input group  "Money Management"
input ushort   InpStopLoss       = 60;       // Stop Loss, in pips (1.00045-1.00055=1 pips)
input ushort   InpTakeProfit     = 500;       // Take Profit, in pips (1.00045-1.00055=1 pips)
input bool     InpUseTrailing    = true;     // Use Trailing stop
input ushort   InpTrailingStop   = 35;       // Trailing Stop (min distance from price to Stop Loss, in pips
...
double ExtCandleLength = 0.0;
double ExtRetracementLength = 0.0;
string ExtComment = "";

ENUM_TIMEFRAMES InpCandleTF =PERIOD_CURRENT;// Candle TF
...
//--- tuning for 3 or 5 digits
   ...
   ExtMinDistance    = InpChannelDistance * m_adjusted_point;
   ExtRetracementLength    = InpRetracementLength * m_adjusted_point;
...
void CheckSignal(bool &buy_signal,bool &sell_signal)
  {
   ...
   double sl=0.0;
   double tp=0.0;
   uchar noOfSellPositions = 0;
   uchar noOfBuyPositions = 0;

   int highest_candle,lowest_candle;
   MqlRates rates[];
   MqlRates candlesInfo[];
   double high[],low[];

   ArraySetAsSeries(rates,true);
   ArraySetAsSeries(rates,true);
   ArraySetAsSeries(candlesInfo,true);
   ArraySetAsSeries(high,true);
   ArraySetAsSeries(low,true);


   CopyHigh(_Symbol,InpChannelTF,InpCandleShift,InpNoOfChannelBars,high);
   CopyLow(_Symbol,InpChannelTF,InpCandleShift,InpNoOfChannelBars,low);

   int start_pos=0,count=10, ratescount=100;

   if(CopyRates(_Symbol,InpChannelTF,InpCandleShift,InpNoOfChannelBars,rates) < InpNoOfChannelBars||
      CopyRates(_Symbol,InpCandleTF,0,count,candlesInfo) < count)
     {
      return;
     }

   highest_candle = ArrayMaximum(high,0,InpNoOfChannelBars);
   lowest_candle = ArrayMinimum(low,0,InpNoOfChannelBars);

   double highest_price = rates[highest_candle].high + ExtMinDistance;
   double lowest_price = rates[lowest_candle].low - ExtMinDistance;
   double averageprice = (candlesInfo[2].low + candlesInfo[2].high)/2;
   ...
   bool goodSell = MathAbs(highest_price - candlesInfo[1].low) > ExtRetracementLength;
   bool goodBuy = MathAbs(lowest_price - candlesInfo[1].high) > ExtRetracementLength;

   if(count_sells < InpMaxSellPositions && candlesInfo[2].high > highest_price && goodSell && !InpReverseSells)
     {
      DeleteAllPendingOrders();
      sell_signal = true;
     }

   if(count_buys < InpMaxBuyPositions && candlesInfo[2].high > highest_price && goodSell && InpReverseSells)
     {
      DeleteAllPendingOrders();
      buy_signal = true;
     }

   if(count_buys < InpMaxBuyPositions && candlesInfo[2].low < lowest_price && goodBuy && !InpReverseBuys)
     {
      DeleteAllPendingOrders();
      buy_signal = true;
     }

   if(count_sells < InpMaxSellPositions && candlesInfo[2].low < lowest_price && goodBuy && InpReverseBuys)
     {
      DeleteAllPendingOrders();
      sell_signal = true;
     }

   if(InpAddPendingOrders && !IsPendingOrdersExists() && candlesInfo[2].high > highest_price && goodSell)
     {
      price=candlesInfo[2].low - ExtMinDistance;
      sl=(InpStopLoss==0)?0.0:price+ExtStopLoss;
      tp=(InpTakeProfit==0)?0.0:price-ExtTakeProfit;
      PendingOrder(ORDER_TYPE_SELL_STOP, price,sl,tp,ExtComment);
     }

   if(InpAddPendingOrders && !IsPendingOrdersExists() && candlesInfo[2].low < lowest_price && goodBuy)
     {     
      price=candlesInfo[2].high + ExtMinDistance;
      sl=(InpStopLoss==0)?0.0:price-ExtStopLoss;
      tp=(InpTakeProfit==0)?0.0:price+ExtTakeProfit;
      PendingOrder(ORDER_TYPE_BUY_STOP, price,sl,tp,ExtComment);
     }
