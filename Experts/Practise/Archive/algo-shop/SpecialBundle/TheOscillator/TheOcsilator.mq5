...
#property description   "The Oscilator"
//---
...
input string   inpComment           = "The Oscillator"; //Trade Comment
...
input int      InpChannelDistance      = 40;     // Order Limit Distance
input int      InpLowerChannelDistance      = 35;     // Stop Order Distance
input bool     InpAddPendingOrders     = true;     // Add extra pending orders

input group  "Money Management"
input ushort   InpStopLoss       = 115;       // Stop Loss, in pips 
input ushort   InpTakeProfit     = 500;       // Take Profit, in pips 
input bool     InpUseTrailing    = true;     // Use Trailing stop
input ushort   InpTrailingStop   = 35;       // Trailing Stop, in pips
input ushort   InpTrailingStep   = 6;        // Trailing Step, in pips 
...
input ENUM_TIMEFRAMES      Inp_MA_TF =PERIOD_CURRENT;    // Moving Average Timeframe
...
input double            InpRSILevelDOWN      = 20;          // RSI Level DOWN
...
double ExtLowerMinDistance = 0.0 ;
string ExtComment = "";
...
//--- tuning for 3 or 5 digits
   ...
   ExtLowerMinDistance    = InpLowerChannelDistance * m_adjusted_point;
...
//--- create handle of the indicator iMA
   handle_iMA_Slow=iMA(m_symbol.Name(),Inp_MA_TF,Inp_MA_Slow_ma_period,Inp_MA_ma_shift,
...
void CheckSignal(bool &buy_signal,bool &sell_signal)
  {
   ...
   if(count_sells < InpMaxSellPositions && rsi[1] > InpRSILevelUP && rsi[2] >  ma_slow[2] && rsi[1]<  ma_slow[1] && ma_slow[1]> InpRSILevelUP)
     {
      DeleteAllPendingOrders();
      sell_signal = true;
      if(InpAddPendingOrders)
        {
         price= rates[1].high + ExtMinDistance;
         sl=(InpStopLoss==0)?0.0:price+ExtStopLoss;
         tp=(InpTakeProfit==0)?0.0:price-ExtTakeProfit;
         PendingOrder(ORDER_TYPE_SELL_LIMIT, price,sl,tp,ExtComment);

         price= rates[1].low - ExtLowerMinDistance;
         sl=(InpStopLoss==0)?0.0:price+ExtStopLoss;
         tp=(InpTakeProfit==0)?0.0:price-ExtTakeProfit;
   ...
   if(count_buys < InpMaxBuyPositions && rsi[1] < InpRSILevelDOWN && rsi[2] <  ma_slow[2] && rsi[1]>  ma_slow[1] && ma_slow[1]<InpRSILevelDOWN)
     {
      DeleteAllPendingOrders();
      buy_signal = true;
      if(InpAddPendingOrders)
        {
         price= rates[1].low - ExtMinDistance;
         sl=(InpStopLoss==0)?0.0:price-ExtStopLoss;
         tp=(InpTakeProfit==0)?0.0:price+ExtTakeProfit;
         PendingOrder(ORDER_TYPE_BUY_LIMIT, price,sl,tp,ExtComment);

         price= rates[1].high + ExtLowerMinDistance;
         sl=(InpStopLoss==0)?0.0:price-ExtStopLoss;
         tp=(InpTakeProfit==0)?0.0:price+ExtTakeProfit;
