//+------------------------------------------------------------------+
//|                                             BollingerScalper.mq5 |
//|                                      Copyright 2019, Algotrading |
//|                                         http://algotrading.co.za |
//+------------------------------------------------------------------+
#property copyright "Copyright 2019, Algotrading"
#property link      "http://algotrading.co.za"
#property version   "1.00"
#property description   "Bollinger Scalper"
//---
...
input string   inpComment           = "Bollinger Scalper"; //Trade Comment
input ulong    m_magic=122320191424;                 // magic number
...
input ushort   InpTrailingStop   = 7;       // Trailing Stop, in pips
input ushort   InpTrailingStep   = 7;        // Trailing Step, in pips
...
input ENUM_MA_METHOD       Inp_MA_Fast_ma_method=MODE_EMA;// MA smoothing type
input ENUM_APPLIED_PRICE   Inp_MA_Fast_applied_price=PRICE_CLOSE;// MA type of price
input ENUM_TIMEFRAMES      Inp_MA_Fast_TF =PERIOD_CURRENT;    // Moving Average Timeframe

input group  "Stochastic Settings"
input int               InpKperiod        = 20;              // Stochastic: K-period (number of bars for calculations)
...
//--- create handle of the indicator iMA
   handle_iMA_Fast=iMA(m_symbol.Name(),Inp_MA_Fast_TF,Inp_MA_Fast_ma_period,Inp_MA_Fast_ma_shift,
...
//--- if the handle is not created
   ...
   handle_iStochastic=iStochastic(m_symbol.Name(),Inp_Stoch_TF,
...
//--- create handle of the indicator iBands
   handle_iBands=iBands(m_symbol.Name(),Inp_Bollinger_TimeFrame,Inp_Bollinger_Ma_Period,Inp_Bollinger_ma_shift,Inp_Bollinger_Deviation,Inp_Bollinger_applied_price);
//--- if the handle is not created
...
void CheckSignal(bool &buy_signal,bool &sell_signal)
  {
   ...
   MqlRates rates[];
   double ma_slow[],stoch[], ma_fast[],bands_upper_array[],bands_lower_array[];
   ...
   if(!iGetArray(handle_iMA_Slow,0,start_pos,count,ma_slow)||
      ...
      !iGetArray(handle_iStochastic,MAIN_LINE,start_pos,count,stoch) ||
      !iGetArray(handle_iBands,UPPER_BAND,start_pos,count,bands_upper_array) ||
   ...
   if(count_sells < InpMaxSellPositions && ma_fast[1] < ma_slow[1]  && rates[2].high > bands_upper_array[2]&& rates[1].low < bands_upper_array[1] && stoch[2] > InpLevelUP && stoch[1] < InpLevelUP)
     {
      sell_signal = true;
     }

   if(count_buys < InpMaxBuyPositions && ma_fast[1]> ma_slow[1]   && rates[2].low < bands_lower_array[2]&& rates[1].high > bands_lower_array[1] && stoch[2] < InpLevelDOWN && stoch[1] > InpLevelDOWN)
     {
      buy_signal = true;
     }
  }
